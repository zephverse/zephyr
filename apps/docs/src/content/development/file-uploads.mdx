...
export function MediaAttachment({ attachment }: { attachment: Media }) {
  // Derive display name from storage key (basename)
  const displayName = attachment.key.split('/').pop() || 'file';

  if (attachment.type === "IMAGE") {
    return <img src={attachment.url} alt={displayName} className="rounded-lg" />;
  }
  ...
  return <a href={attachment.url} download>{displayName}</a>;
}
```
