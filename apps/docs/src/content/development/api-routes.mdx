...
Creates a new post with content, optional media attachments, tags, and mentions.

```typescript
// Request body
{
  content: string,
  mediaIds?: string[],  // Optional array of media IDs
...
**POST** - Revalidate specific cache keys (admin only)

⚠️ **Restricted Endpoint**: This endpoint is not publicly accessible and requires special authorization.

**Authorization Requirements:**
- Must include `Authorization` header with valid bearer token
- Requires `admin` role or `cache:revalidate` permission
- Limited to authenticated admin users or server-to-server calls
- CSRF protection not applicable (server-to-server only)
...
See [Cron Jobs](/development/cron-jobs) for detailed documentation.
