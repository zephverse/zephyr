      ...
      <div className="flex h-full animate-pulse">
        <div className="w-80 border-border border-r bg-background">
          ...
          <div className="space-y-4 p-4">
            {["chat-1", "chat-2", "chat-3", "chat-4", "chat-5"].map((id) => (
        ...
        <div className="flex flex-1 flex-col">
          ...
          <div className="flex-1 space-y-4 p-4">
            {["msg-1", "msg-2", "msg-3", "msg-4"].map((id, index) => (
              <div
                className={`flex items-end space-x-2 ${index % 2 === 0 ? "justify-end" : ""}`}
                key={id}
              >
                {index % 2 !== 0 && (
                ...
                <div
                  className={`space-y-2 ${index % 2 === 0 ? "items-end" : "items-start"}`}
                >
                  <div
                    className={`h-4 w-48 rounded-sm bg-muted ${index % 2 === 0 ? "ml-auto" : ""}`}
                  />
                  <div
                    className={`h-4 w-32 rounded-sm bg-muted ${index % 2 === 0 ? "ml-auto" : ""}`}
                  />
                </div>
                {index % 2 === 0 && (
                  <div className="h-8 w-8 rounded-full bg-muted" />
